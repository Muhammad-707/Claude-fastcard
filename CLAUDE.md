...
### 📥 Команда "stop" или "стоп" (Сохранение и выход)
Если пользователь пишет в чат `stop` или `стоп`, ты обязан:
1. Остановить текущую генерацию.
2. В самый конец этого файла (`CLAUDE.md`) в раздел `## 📍 Последний чекпоинт` записать человеческим языком: какую конкретно страницу, компонент или фичу ты делал в эту секунду и какой следующий шаг нужно сделать.
3. Запустить `npm run lint`.
4. Выполнить команду: `git add .`
5. Сделать коммит: `git commit -m "checkpoint: paused on [Название фичи/страницы]"`
6. Выполнить: `git push origin main`
7. Попрощаться и завершить сессию.

### 🚀 Команда "start" или "старт" (Возобновление работы)
Если пользователь пишет в чат `start` или `старт`, ты обязан:
1. Первым делом прочитать раздел `## 📍 Последний чекпоинт` внизу этого файла.
2. Посмотреть текущий статус файлов в проекте и git-лог.
3. Молча, без лишних вопросов, продолжить разработку ровно с того места, где остановился в прошлый раз, используя макеты из Figma MCP и правила Taste Skill.
