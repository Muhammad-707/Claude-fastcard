  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-[4px] bg-[#F5F5F5]">
        ...
        {/* Badge */}
        {showBadge === 'new' && (
          <span className="absolute left-2 top-2 rounded-[4px] bg-[#00FF66] px-2.5 py-1 text-[10px] font-semibold text-black sm:left-3 sm:top-3 sm:px-3 sm:text-xs">
            NEW
          </span>
        )}
        {(showBadge === 'sale' || (!showBadge && discountPct)) && discountPct && (
          <span className="absolute left-2 top-2 rounded-[4px] bg-[#DB4444] px-2.5 py-1 text-[10px] font-semibold text-white sm:left-3 sm:top-3 sm:px-3 sm:text-xs">
            -{discountPct}%
          </span>
        )}
        ...
        <div className="absolute right-2 top-2 flex flex-col gap-1.5 opacity-0 transition-opacity duration-200 group-hover:opacity-100 sm:right-3 sm:top-3 sm:gap-2">
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-md text-foreground hover:text-[#DB4444] transition-colors sm:h-9 sm:w-9"
      ...
      <div className="mt-2 space-y-1 sm:mt-3 sm:space-y-1.5">
        <p className="text-xs font-medium text-foreground line-clamp-2 sm:text-sm">{product.productName}</p>
        ...
        {/* Stars — hide on smallest screens to save space */}
